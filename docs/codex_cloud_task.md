...
2. Improve long-term storage and session recovery.
   ...
   - The current project has segmented CSV storage, session index recovery, stale active-session recovery, retention/delete controls, resume preparation with source-session lineage metadata, atomic segment-index replacement, saved-log metadata reconciliation on Session Manager refresh, target-month bucket summaries, and structured Session Manager filters for faster history browsing.

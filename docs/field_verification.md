...
3. 여러 IPv4 주소를 한 줄에 하나씩 붙여넣고 시작합니다.
...
7. 측정 중 IP 추가, 이름 버튼, 일시중지/삭제 버튼이 해당 대상 행에만 적용되는지 확인합니다.
8. IP별 상태 색상과 시간 범위 선택이 모든 그래프 행에 동일하게 적용되는지 확인합니다.
9. EXE를 배포 폴더가 아닌 다른 작업 폴더에서 실행해도 정상 시작되는지 확인합니다.
...
- 장시간 측정: 그래프는 선택한 시간 범위로 표시되고 `%LOCALAPPDATA%\MultiPingCheck\session_logs`에 세션 샘플 로그가 생성되어야 합니다.
- 손상된 세션: 부분 복구 또는 `SESSION_LOG_CORRUPTED` 오류가 명확히 표시되어야 합니다.
